// cuidarte-web/app/admin/page.js
"use client";
...
export default function AdminDashboard() {
  ...
  const fetchEvents = useCallback(async (startStr, endStr) => {
    ...
    try {
      const data = await apiFetch(
        `/api/servicios/agenda?start=${startStr}&end=${endStr}`,
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && (
        <div className="p-3 text-sm text-gray-600">Cargando eventos…</div>
      )}
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-[#1D4A74]">
              ...
              <div>
                ...
                <div>
                  ...
                  {selected?.profesional?.profesion
                    ? ` — ${selected.profesional.profesion}`
                    : ""}
                </div>
              </div>
